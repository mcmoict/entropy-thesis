...
class Worker:
    ...
    def __init__(
        ...
        *,
        ...
        # Exact DES congestion events generated by TrafficController for this
        # worker.  This is a reference mirror of TrafficController.wait_events;
        # it does not recalculate or alter the thesis congestion KPI.
        ...
        for from_node, to_node in pairwise(route.nodes):
            ...
            if self.traffic_controller is None:
                ...
                with resource.request() as request:
                    yield request
                    started_at = float(self.env.now)
                    wait_event_start = len(self.traffic_controller.wait_events)
                    ...
                    # record_edge_wait appends only the actual positive-wait
                    # contention event(s). Keep the exact same objects here so
                    # visualization can recover the event timestamp later.
                    if len(self.traffic_controller.wait_events) > wait_event_start:
                        self.wait_events.extend(
                            self.traffic_controller.wait_events[wait_event_start:]
                        )
                    yield self.env.timeout(travel_seconds)
                    finished_at = float(self.env.now)
    ...
    def pick_task(self, task: PickTask):
        ...
        if self.traffic_controller is None:
            ...
            with resource.request() as request:
                ...
                # Same event object used by Phase 2/3 summary. No additional
                # conflict definition or approximation is introduced.
                if len(self.traffic_controller.wait_events) > wait_event_start:
                    self.wait_events.extend(
                        self.traffic_controller.wait_events[wait_event_start:]
                    )
