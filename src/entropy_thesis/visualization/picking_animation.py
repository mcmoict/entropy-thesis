...
<style>
  * {{ box-sizing: border-box; }}
  ...
  #overlay .marker circle {{ stroke: #fff; stroke-width: 2.2; transition: fill .08s linear, stroke .08s linear, stroke-width .08s linear; }}
  ...
  <div class="layout">
    ...
    <aside class="sidebar">
      ...
      <div class="notes">
        ...
        - 두 피커가 근접하여 충돌로 판정되면 해당 마커가 빨간색으로 표시됩니다.<br />
...
(() => {{
  ...
  // SVG 좌표 기준 충돌 판정 거리. 두 마커 중심이 이 값 이하로 가까우면
  // 충돌로 표시한다. 둘 다 idle인 경우(예: I/O 지점 대기)는 제외한다.
  const COLLISION_DISTANCE_PX = 10.0;
  ...
  function rebuildWorkers() {{
    ...
    scenario.workers.forEach((worker, index) => {{
      ...
      markerMap.set(worker.worker_id, {{ g, c, t, dot, baseColor: color }});
    }});
    selectedWorker = 'ALL';
  }}
  ...
  function render() {{
    ...
    // 먼저 모든 작업자의 현재 위치를 계산한다. 작업자 한 명만 화면에 표시하는
    // 경우에도, 숨겨진 다른 작업자와의 충돌 여부는 정상적으로 계산된다.
    const frameStates = scenario.workers.map(worker => {{
    ...
    const threshold2 = COLLISION_DISTANCE_PX * COLLISION_DISTANCE_PX;

    for (let i = 0; i < frameStates.length; i++) {{
      const a = frameStates[i];
      for (let j = i + 1; j < frameStates.length; j++) {{
        const b = frameStates[j];

        // 출발/종료 I/O 등에서 대기 중인 작업자끼리 겹쳐 있는 것은
        // 실제 이동 충돌로 보지 않는다.
        if (a.seg.kind === 'idle' && b.seg.kind === 'idle') continue;

        const dx = a.p.x - b.p.x;
        const dy = a.p.y - b.p.y;
        if ((dx * dx + dy * dy) <= threshold2) {{
          collisionWorkers.add(a.worker.worker_id);
          collisionWorkers.add(b.worker.worker_id);
          collisionPairs.push(`${{a.worker.worker_id}}↔${{b.worker.worker_id}}`);
        }}
      }}
    }}

    const states = [];
    frameStates.forEach(({{ worker, seg, p }}) => {{
      ...
      marker.c.setAttribute('fill', marker.baseColor);
    ...
    const collisionText = collisionPairs.length
      ? collisionPairs.slice(0, 5).join(', ') + (collisionPairs.length > 5 ? ' ...' : '')
      : '없음';
    ...
    setStatus(`<strong>현재 시간</strong> : ${{formatSeconds(currentTime)}}<br>` +
      ...
      `<strong>충돌 피커 수</strong> : ${{collisionWorkers.size}}명 · <strong>충돌 쌍</strong> : ${{collisionPairs.length}}<br>` +
      `<strong>충돌</strong> : ${{collisionText}}<br>` +
