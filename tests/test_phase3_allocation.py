...
def test_build_aisle_zones_uses_contiguous_support_aisles():
    warehouse = build_tiny_warehouse()
    zones = build_aisle_zones(warehouse, number_of_zones=2)

    assert [zone.zone_id for zone in zones] == ["Z01", "Z02"]
    assert zones[0].aisle_y_values == (0.0,)
    assert zones[1].aisle_y_values == (1.0,)


def test_classification_keeps_list_intact_and_uses_dominant_zone():
    warehouse = build_tiny_warehouse()
    zones = build_aisle_zones(warehouse, number_of_zones=2)
    lists = [
    ...
    assert assignments[1].zone_id == "Z02"
    assert zone_workload(zones, assignments, basis="tasks") == (3.0, 1.0)
